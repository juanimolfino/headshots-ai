...
- Failed job rows can be permanently deleted with the X/delete action. This removes the database row for the owning user instead of hiding it locally.
...
- The OG/Twitter placeholder image is generated by `app/opengraph-image.tsx` and reused by `app/twitter-image.tsx`; it now includes the Pic Your AI logo but is still placeholder brand art.
