...
5. Worker `runAiJob` processes the job.
  ...
  - Headshot training/generation is moving from the old single PhotoMaker job to a two-step Flux LoRA flow: train with `headshot-training`, then generate with `headshot-generate`.
