...
- **fal.ai**: demo image provider using Flux Schnell, headshot generation through PhotoMaker, and public temporary uploads through fal.storage. Adapters live in `lib/ai/providers/fal.ts` and `lib/ai/providers/photomaker.ts`; upload route is `app/api/upload/route.ts`.
...
5. Worker `runAiJob` processes the job.
   ...
   - For headshot jobs, `lib/ai/providers/photomaker.ts` calls `fal-ai/photomaker` through the fal queue API. Current worker support is still provisional: it serializes returned image URLs as JSON until the dedicated headshot worker flow is implemented.
...
- `jobs`
  - `id`: UUID primary key.
  - `userId`: FK to `users.id`.
  - `type`: enum `image`, `tts`, or `headshot`.
  - `status`: enum `pending`, `processing`, `done`, `failed`.
  - `input`: JSON input payload. For image it stores `{ prompt }`; for TTS it stores `{ text, voice }`; for headshot it stores `{ archive_url, style, num_images }`.
...
- Headshot job type in Drizzle/Postgres (`job_type = headshot`) and request validation.
...
- Headshot workflow is incomplete end-to-end: upload works and the provider is registered, but there is no ZIP-building worker step yet and no headshot-specific dashboard result rendering.
...
5. Complete headshot generation flow.
   - Build the ZIP assembly step from uploaded fal.storage image URLs.
   - Decide whether ZIP construction happens client-side, in an API route, or in Inngest.
   - Update the Inngest worker to handle `headshot` explicitly instead of storing PhotoMaker URLs as JSON.
   - Store/render multiple generated headshot image URLs in a product-ready shape.
