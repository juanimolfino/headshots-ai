...
5. Worker `runAiJob` processes the job.
   ...
   - For headshot jobs, `lib/inngest/functions.ts` parses uploaded fal.storage image URLs, builds a ZIP in memory with JSZip, uploads the ZIP to fal.storage, calls `lib/ai/providers/photomaker.ts`, downloads the generated fal.ai images, uploads them to Supabase Storage under `headshots/{userId}/{jobId}/{index}.jpg`, stores the array in `jobs.result`, and stores the first image URL in `jobs.result_url` for compatibility.
...
5. Complete headshot generation flow.
   - Run a real end-to-end PhotoMaker job in Inngest and verify generated images in Supabase Storage.
